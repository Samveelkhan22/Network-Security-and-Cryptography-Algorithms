import numpy as np

# AES S-box
s_box = np.array([
    ], dtype=np.uint8)

# AES Inverse S-box
inv_s_box = np.array([
    ], dtype=np.uint8)

# AES Rijndael Galois Field (GF) Multiplication
def gf_mul(a, b):
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        high_bit_set = a & 0x80
        a <<= 1
        if high_bit_set:
            a ^= 0x1b
        b >>= 1
    return p

# Key expansion for AES
def key_expansion(key):
    round_constants = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36]
    rcon_idx = 0
    round_keys = [key]
    for i in range(10):
        round_key = []
        prev_key = round_keys[-1]
        # Rotate word
        temp = prev_key[1:] + prev_key[:1]
        # Substitute bytes
        temp = np.array([s_box[b] for b in temp], dtype=np.uint8)
        # XOR with round constant
        temp[0] ^= round_constants[rcon_idx]
        rcon_idx += 1
        # XOR with previous word
        temp = np.bitwise_xor(temp, np.array(prev_key[0:4], dtype=np.uint8))
        round_key.extend(temp)
        # Generate next three words
        for j in range(1, 4):
            temp = np.bitwise_xor(temp, np.array(prev_key[j * 4:(j + 1) * 4], dtype=np.uint8))
            round_key.extend(temp)
        round_keys.append(round_key)
    return round_keys

# SubBytes step of AES
def sub_bytes(state):
    return np.array([s_box[b] for b in state], dtype=np.uint8)

# ShiftRows step of AES
def shift_rows(state):
    state = state.reshape(4, 4)
    for i in range(1, 4):
        state[i] = np.roll(state[i], -i)
    return state.flatten()

# MixColumns step of AES
def mix_columns(state):
    state = state.reshape(4, 4)
    for i in range(4):
        s0 = state[i][0]
        s1 = state[i][1]
        s2 = state[i][2]
        s3 = state[i][3]

        state[i][0] = gf_mul(s0, 2) ^ gf_mul(s1, 3) ^ s2 ^ s3
        state[i][1] = s0 ^ gf_mul(s1, 2) ^ gf_mul(s2, 3) ^ s3
        state[i][2] = s0 ^ s1 ^ gf_mul(s2, 2) ^ gf_mul(s3, 3)
        state[i][3] = gf_mul(s0, 3) ^ s1 ^ s2 ^ gf_mul(s3, 2)

    return state.flatten()

# AddRoundKey step of AES
def add_round_key(state, round_key):
    return state ^ round_key

# Encrypt plaintext with AES
def encrypt(plaintext, key):
    key = bytearray(key)
    round_keys = key_expansion(key)
    state = np.frombuffer(plaintext.encode(), dtype=np.uint8)
    state_len = len(state)
    padded_len = 16 - (state_len % 16)
    state = np.append(state, [0] * padded_len)
    state_len += padded_len

    ciphertext = bytearray()

    for i in range(0, state_len, 16):
        block = state[i:i + 16]
        block = add_round_key(block, round_keys[0])

        for round_key in round_keys[1:]:
            block = sub_bytes(block)
            block = shift_rows(block)
            block = mix_columns(block)
            block = add_round_key(block, round_key)

        ciphertext += block.tobytes()

    return ciphertext

# Decrypt ciphertext with AES
def decrypt(ciphertext, key):
    key = bytearray(key)
    round_keys = key_expansion(key)
    state = np.frombuffer(ciphertext, dtype=np.uint8)

    plaintext = bytearray()

    for i in range(0, len(state), 16):
        block = state[i:i + 16]

        for round_key in reversed(round_keys[1:]):
            block = add_round_key(block, round_key)
            block = np.array([inv_s_box[b] for b in block], dtype=np.uint8)
            block = np.roll(block.reshape(4, 4), -1, axis=0).flatten()
            block = np.roll(block.reshape(4, 4), -2, axis=0).flatten()
            block = np.roll(block.reshape(4, 4), -3, axis=0).flatten()

        block = add_round_key(block, round_keys[0])

        plaintext += block.tobytes()

    return plaintext.strip(b'\x00').decode()

# Example usage
key = b'Sixteen byte key'
plaintext = "This is a secret message"

# Encryption
ciphertext = encrypt(plaintext, key)
print("Ciphertext:", ciphertext)

# Decryption
decrypted_text = decrypt(ciphertext, key)
print("Decrypted text:", decrypted_text)
